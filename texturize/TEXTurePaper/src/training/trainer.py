...
class TEXTure:
    def __init__(self, cfg: TrainConfig):
        self.cfg = cfg
        self.paint_step = 0
        self.device = self.cfg.device # torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        utils.seed_everything(self.cfg.optim.seed)

        # Make view_dirs
        self.exp_path = make_path(self.cfg.log.exp_root/self.cfg.log.exp_name)
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        # THIS STEP PRONE TO OVER-REQUESTING MEMORY -- cross attention is too much.
        with torch.no_grad(): 
